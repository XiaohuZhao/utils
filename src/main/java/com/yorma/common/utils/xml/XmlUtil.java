...
public class XmlUtil {
     ...
     * @param xmlObj 要转成xml字符串的对象
     * @return xml格式的字符串
     * @throws JAXBException 创建JAXBContext实例时出现异常 可能是注解不正确
     */
    ...
    public static String xmlToJson(String xmlString){
        return XML.toJSONObject(xmlString.replace("<xml>", "").replace("</xml>", "")).toString();
    }
    ...
    public static String json2xml(String jsonString) {
        return "<xml>" + XML.toString(new JSONObject(jsonString)) + "</xml>";
    }
    
    /**
     * 将xml格式的字符串转化成实体对象
     * @param xmlStr xml格式的字符串
     * @param tClass 要转化成的实体类型
     * @param <T> 实体类型泛型
     * @return 转化成的实体
     */
     ...
     * @param str xml字符串
     * @return 格式化后的字符串
     * @throws DocumentException xml字符串格式不正确
